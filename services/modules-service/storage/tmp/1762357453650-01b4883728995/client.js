class EventureModuleRoot extends HTMLElement {
  ...
  async connectedCallback() {
    ...
    let title = "People";
    try {
      const cfg = await fetch(`/api/modules/slug/${slug}/config`, { cache: "no-store" }).then(r=>r.json());
      if (cfg?.config?.title) title = String(cfg.config.title);
    } catch {}
